...
// create variable equal to return of getCars function, when referring to cars array we will use "array"
//      as a standin for "cars"
let array = getCars();
...
const createContent = () => {
  ...
  $("#createButton").on("click", () => {
    ...
    let userActivity = `<div class = "userActivity"> 
            Added a ${userSubmittedCar.carColor} ${userSubmittedCar.carYear} ${userSubmittedCar.carMake} ${userSubmittedCar.carModel} with ${userSubmittedCar.carMiles} miles at $${userSubmittedCar.carPrice} <br>
        </div>`;
...
const readContent = (event) => {
  ...
  infoBarContent = `This ${filteredCars[index].carYear} ${filteredCars[index].carMake} ${filteredCars[index].carModel} is a ${filteredCars[index].carColor} ${filteredCars[index].carType} with ${filteredCars[index].carMiles} miles on it, for a price of $${filteredCars[index].carPrice}. <br> 
...
const updateContent = (event) => {
  ...
  $("#updateButton").on("click", () => {
    ...
    let userActivity = `<div class = "userActivity"> 
            Updated a car in position of ${index + 1} to be a ${
      array[index].carYear
    } ${array[index].carMake} ${array[index].carModel} with ${
      array[index].carMiles
    } miles at $${array[index].carPrice} <br>
        </div>`;
    $(".activityContainer").append(userActivity);
...
const deleteContent = (event) => {
  ...
  let userActivity = `<div class = "userActivity"> 
            Deleted a ${array[index].carColor} ${array[index].carYear} ${array[index].carMake} ${array[index].carModel} with ${array[index].carMiles} miles at $${array[index].carPrice} <br>
        </div>`;
  $(".activityContainer").append(userActivity);
...
const filterContent = () => {
  ...
  let userCarYear = $(`<div class = "userCarYear">
        ...
        <select name = "year" id = "yearID">
            ...
            <option value="andBefore">And Earlier</option>
            <option value="andAfter">And Later</option>
        </select>   
    </div>`);
  ...
  $("#filterButton").on("click", () => {
    ...
    array.forEach((car) => {
      ...
      if (userSubmittedYear > 0) {
        ...
        } else if ($("#yearID").val() == "andBefore") {
          yearCheck = car.carYear <= userSubmittedYear;
        } else if ($("#yearID").val() == "andAfter") {
          yearCheck = car.carYear >= userSubmittedYear;
        }
      }
      ...
      if (userSubmittedMiles > 0) {
        if ($("#milesID").val() == "andBelow") {
          milesCheck = car.carMiles <= userSubmittedMiles;
        } else if ($("#milesID").val() == "andAbove") {
          milesCheck = car.carMiles >= userSubmittedMiles;
        }
      }
      ...
      if (userSubmittedPrice > 0) {
        if ($("#priceID").val() == "andBelow") {
          priceCheck = car.carPrice <= userSubmittedPrice;
        } else if ($("#priceID").val() == "andAbove") {
          priceCheck = car.carPrice >= userSubmittedPrice;
        }
      }
